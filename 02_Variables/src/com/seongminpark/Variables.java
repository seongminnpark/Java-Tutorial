...
public class Variables {
	...
	public static void main(String args[]) {
		...
		/* As you'll notice a variable consists of three parts: type, name and value.
		   ...
		   Declaring a variable, then, means writing out the three components. 
		   We also add an equals sign(=) to show it's an assignment. 
		 ...
		 * 
		   ...
		   2. Since our special number is declared only once in the code, we only have to change that single line to 
		      effect every line in which the variable is mentioned.
		   ...
		   3. We may not know the value we need to print as we are writing the program.
		      For example, we might want to ask the user to input a number. Or we might want to perform specific operations 
		      to the value before printing it out, like in the 4th print in the example code. 
		...
		System.out.format("Integer Division: %d%n",sixInt / 4);
		/* Use a float or a double if you want *relatively* precise devision. */
		System.out.format("Decimal Point Division: %f%n",sixDouble / 4);
		
		/* Final bit on numbers: since numbers usually involve arithmetic operations and value updates,
		   these syntax were added to combine each arithmetic operation with the assignment operator. */
		int manipulateMe = 100;
		manipulateMe += 1; // Same as: manipulateMe = manipulateMe + 1; 
		manipulateMe -= 2; // Same as: manipulateMe = manipulateMe - 2; 
		manipulateMe *= 3; // Same as: manipulateMe = manipulateMe * 3; 
		manipulateMe /= 9; // Same as: manipulateMe = manipulateMe / 9; 
		System.out.format("manipulateMe: %d%n", manipulateMe); // Guess what this will print.
